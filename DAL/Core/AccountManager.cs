...
namespace DAL.Core
{
    public class AccountManager : IAccountManager
    {
        ...
        public async Task<(ApplicationUser User, string[] Roles, List<Claim> Claims)?> GetUserAndRolesAsync(string userId)
        {
            ...
            var userClaimIds = user.Claims.Select(r => r.Id).ToList();


            var claims = _context.UserClaims
               .Where(r => userClaimIds.Contains(r.Id))
        ...
        public async Task<(bool Succeeded, string[] Errors)> UpdateUserAsync(ApplicationUser user, IEnumerable<string> roles, IEnumerable<Claim> claims)
        {
            var result = await _userManager.UpdateAsync(user);
            if (!result.Succeeded)
                return (false, result.Errors.Select(e => e.Description).ToArray());
            ...
            if (claims != null)
            {
                ...
                var claimsToRemove = userClaims.Except(claims).ToArray();
                var claimsToAdd = claims.Except(userClaims).Distinct().ToArray();
